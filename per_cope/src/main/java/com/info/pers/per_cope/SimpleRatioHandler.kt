...
object SimpleRatioHandler : RatioHandler {
    private val rn = "supa"
    ...
    override suspend fun e(d: String, h: String): Info? {
    ...
    override fun ku(ew: Context): Boolean {
        val gre = try {
            val sgd =
    ...
    override suspend fun lfe(grg: Context, vx: Data) {
    ...
    override suspend fun b(w: Context): Data {
        val e = w.getSharedPreferences(q, Context.MODE_PRIVATE)

        return withContext(Dispatchers.IO) {
            val asg = e.getString(this@SimpleRatioHandler.w, null)
    ...
    override suspend fun y(ge: Context, dgv: String) {
        if (h != null) return
        ...
        lfe(ge, Data(dgv, false))
    }

    @SuppressLint("SetJavaScriptEnabled")
    @Suppress("DEPRECATION")
    override fun o(fw: WebView) {
        with(fw.settings) {
    ...
    override fun rg(dgd: Class<out Activity>) {
        f = dgd
    }

    override fun ht(sg: Activity) {
        f?.let { f ->
            CoroutineScope(Dispatchers.IO).launch {
                lfe(sg, Data(null, true))
            }
    ...
    override fun gew(w: Context, s: String) {
        if (s.isBlank()) return
    ...
    override suspend fun fg(geg: Activity, sdv: Info): String {
        with(CoroutineScope(coroutineContext)) {
            ...
            val dd =
                async { getFacebookCampaign(geg, sdv.g, sdv.h) }
    ...
    private suspend fun getFacebookCampaign(ge: Context, wew: String, asdas: String): String? {
        return withContext(Dispatchers.IO) {
